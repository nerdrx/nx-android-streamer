...
| `client-android/.../org/webrtc/NxLowLatencyDecoderFactory.kt` | [moonlight-android](https://github.com/moonlight-stream/moonlight-android) | GPL-3.0 | The technique, not the code: put MediaCodec in low-latency mode for streaming (`KEY_LOW_LATENCY` plus the older vendor keys some decoders still honour) instead of accepting libwebrtc's smoothness-first defaults. Implemented here by injecting a `MediaCodecWrapper` into libwebrtc's `AndroidVideoDecoder`, which is why the file lives in package `org.webrtc`. |
